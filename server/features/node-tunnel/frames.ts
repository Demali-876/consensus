...
export function decodeFrame(raw: Buffer): FrameParts {
  ...
  return {
    ...
    sequence,
    nonce,
    ciphertext: raw.subarray(ciphertextStart, ciphertextEnd),
    tag: raw.subarray(ciphertextEnd, ciphertextEnd + TAG_SIZE),
  };
}
...
function validateParts(parts: FrameParts): void {
  if (parts.version !== FRAME_VERSION) throw new RangeError(`Unsupported frame version: ${parts.version}`);
  if (!VALID_TYPES.has(parts.type)) throw new RangeError(`Unknown frame type: 0x${parts.type.toString(16).padStart(2, '0')}`);
  if (parts.sequence < 0n || parts.sequence > MAX_U64) throw new RangeError('Frame sequence must fit in u64');
  if (parts.nonce.length !== NONCE_SIZE) throw new RangeError(`Frame nonce must be ${NONCE_SIZE} bytes`);
  if (parts.tag.length !== TAG_SIZE) throw new RangeError(`Frame auth tag must be ${TAG_SIZE} bytes`);
  if (parts.ciphertext.length > 0xffffffff) throw new RangeError('Frame ciphertext too large');
}
