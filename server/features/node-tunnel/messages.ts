...
export type EvalAction =
  ...
  // Admission suites (the bench-cpu model): composite = the real per-request
  // pipeline, sustained = windowed steady-state (the admission authority),
  // multicore = effective-core scaling. These replace benchmark_cpu/crypto/memory
  // as the gate; the older primitives remain in the union as diagnostics.
  | 'benchmark_composite'
  | 'benchmark_sustained'
  | 'benchmark_multicore'
  | 'tunnel_echo'
...
export function assertTunnelMessage(value: unknown): asserts value is TunnelMessage {
  if (!value || typeof value !== 'object') throw new TypeError('Tunnel message must be an object');

  const message = value as Record<string, unknown>;
  if (typeof message.type !== 'string') throw new TypeError('Tunnel message type is required');
